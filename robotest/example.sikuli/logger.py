...
class RobotLogger(logging.Logger):
	...
	def screenshot(self, msg="", folder=os.path.join('results','screenshots'), region=(0,0,1440,900)): #"results/screenshots/"
	...
	def passed(self, msg, *args, **kwargs):
		...
		if self.isEnabledFor(logging.DEBUG) and len(getLastFoundImages()) != 0:
			...
			self.screenshot(msg="Best Matches", folder=os.path.join('results','matches'), region=region) #'results/matches/'
			# score of match
